...
# %%
## Bitcoin dataset Preprocessing
import numpy as np
from utils import *

bitcoin_file='data/soc-sign-bitcoinotc.csv'
bitcoin_data={'sources': [], 'targets': [], 'signs': []}
nodes=set()
with open(bitcoin_file, 'r', encoding='utf-8') as f:
  for line in f:
    source, target, rating, time=line.strip('\n').split(',')
    bitcoin_data['sources'].append(source)
    bitcoin_data['targets'].append(target)
    sign=1 if int(rating)>0 else -1
    bitcoin_data['signs'].append(int(sign))
    nodes.add(source)
    nodes.add(target)

str2num=dict((s, i) for i, s in enumerate(nodes))
bitcoin_data['sources']=list(map(lambda s: str2num[s], bitcoin_data['sources']))
bitcoin_data['targets']=list(map(lambda s: str2num[s], bitcoin_data['targets']))
# %%
...
ss, ts, signs=bitcoin_data['sources'], bitcoin_data['targets'], bitcoin_data['signs']
...
accu=evaluate(features, bitcoin_data['signs'])
...
#     if cp=='triad':
#       X, y=sample(features[:,:23], bitcoin_data['signs'], ev)
#     else:
#       X, y=sample(features, bitcoin_data['signs'], ev)
#     if ev not in nEdges:
#       nEdges[ev]=len(y)
...
# print('bitcoin edge embeddedness:')
# for k, v in nEdges.items():
#   print('E=%d: %d edges' %(k, v))

# print('bitcoin result:')
